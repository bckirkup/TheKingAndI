...
| `docs/adr/0071-captivity-and-the-exchange.md` | **D177/D181/D183 wired in the seminar path only:** captures may remain as a private **fengr**, draft **hlutr** clearing prices pay piece cash (the exact ruling is “Pieces first get paid during the draft”), and the cycle-purse ransom round credits captors while exposing only who came home. Hold and benevolence-decay knobs are off/inert by default; campaign and season remain disabled. Exchange hope is now a terminal-only debrief fold. The v1 redemption ordering is harness policy, not doctrine. D182 price-side magnitude is wired inert; live registration, D184, D185, and captor affinity remain open; weregild was considered and rejected because ransom prices a return, not a life, and wælreaf is the foreclosed spoils alternative |
...
| `docs/calibration/2026-09-05-the-recognition-census.md` | **Read before ruling any ADR 0078 recognition threshold:** the Phase A census (`pnpm sim:census`, 2 seeds × 8 weeks × 12 commanders, fake engine). Pride knees at floor 100–250‰, loneliness 25 ≡ 50 and collapses at 75, panic floor 4 is rare-but-present (2 saturates, 6 is silent), relief 500 ≡ 750, survivor guilt at safety floor 0.05 names the supportive room only. Awe, envy, spite, and gratitude are **structural zeros** on this harness (no heroism under the fake engine, no same-role price spread, no ransoms, no costly grievance acts) — not thresholds to tune |
| `docs/calibration/2026-09-05-the-carriers-and-the-floor.md` | **Read before ruling any ADR 0078 carrier magnitude (D208/D211/D212):** the Phase B sweep (9 candidates × 3 styles × 10 campaigns × 20 matches, fake engine, `--opponent=tyrannical`, AWS Batch, seed 314159). Shame fires ≈1/match in cruel rooms and never in kind ones, prices ≈1 trust point (correctly signed, under play noise) because the overridden piece is already at the trust/benevolence floor. Grief fires ≈18/match in the supportive room, 0.2 tyrannical, and as a depth tax is a cost **only the bonded room can pay** — wrong sign for D188 — so suppression is proposed a ruled zero. Bitterness is a **structural zero**: the proportional cliff stalls benevolence at 3 with 47 debt, so `tauBenev <= 0` is unreachable; the owner rules the trigger stays as written (unreachable; a later formation is itself the evidence). Rulings: shame 50‰/witness, grief load 100 / decay 250 with suppression a ruled zero. Adds `shame_exposures`/`grief_mournings`/`bitterness_formations` and end-state loads to the campaign/sweep CSVs |
| `docs/calibration/2026-09-06-the-carriers-that-could-not-fire.md` | **Read before pricing any ADR 0078 Phase C carrier (D215/D216/D217):** the campaign pre-flight finds panic, loneliness, awe, and relief structural zeros in the current world (no heroism under the fake engine; no cruel-room onset at floor 4; bonds only where nobody deserts; relief recipients only below the D207 baseline, where nobody is relieved), and the seed-41 pride seminar grid (`PRIDE_REFUSAL_SCALE` 0/10/50/100/250, AWS Batch) finds the carrier live and play-sensitive but **saturated at scale ≥ 50** (50/100/250 are one play) because appraisals of 125–253‰ land on a `−3..+3` pawn threshold; the graded region is `1..40`. Every Phase C knob stays 0 |
| `docs/calibration/2026-09-08-the-fine-grid-and-the-three-settings.md` | **Read before proposing any nonzero `PRIDE_REFUSAL_SCALE`:** two seeds × fine grid {0,5,10,20,40}; saturation edge is between 10 and 20 (20 = 40 = 50 = 100 = 250 byte-identical), so the knob is three-valued (0/5/10) below it; the sign of every outcome column flips between seeds 41 and 7 (only volatile's refusal-rate drop is consistent, −0.3% to −1.2%); knob stays 0 until the pool prices more pieces. |
| `docs/adr/0078-the-uncarried-emotions.md` | **D208, D209, D211, D212, D213, and D214 wired inert; D210 recognition is partly wired; D215 partly wired for price appraisal, inert; D216 and D217 partly wired for recognition, inert; Phase C live carriers (pride→refusal threshold, panic→engagement collapse, loneliness→stay term, awe→affinity, relief→lift) wired behind zero knobs.** Bitterness is an optional terminal-only carrier with rupture-floor and captive-decay recognition, inert repair/regard/morning-lift discounts, and match-boundary decay. Spite is a match-local, terminal-only recognition fold over existing grievance and act events, with both floors disabled by default. Grief carries clamped deterministic mourning and terminal naming only. Shame is a terminal-only, default-inert reading that reuses ADR 0070 witnesses and scales only the target's own unvindicated-override loss. Guilt is a direct-link, terminal-only fold over desertion cascades and floor-gated peer-safety spend annotations, with zero defaults. Envy is a seminar-only, terminal-only recognition fold over draft settlements, inert at its zero floor. Panic is a terminal-only recognition fold over per-ply capture-risk and King-danger events, inert at its zero floor. Relief is a terminal-only down-crossing capture-risk recognition; awe folds heroism nominations; loneliness folds departed-peer affinity, all inert at their zero floors. Gratitude remains a terminal-only ransom recognition fold; live carriers, thresholds, magnitudes, and pricing remain open |
